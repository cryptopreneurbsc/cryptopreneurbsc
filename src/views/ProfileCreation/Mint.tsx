...
import { Card, CardBody, Heading, Text } from '@plaxswap/uikit'
...
import { MINT_COST, STARTER_NFT_BUNNY_CCF } from './config'
...
const Mint: React.FC = () => {
  ...
  useEffect(() => {
    const getStarterNfts = async () => {
      ...
      const nfts = STARTER_NFT_BUNNY_CCF.map((bunnyId) => {
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% CCF', { num: formatUnits(MINT_COST) })}
          </Text>
          {starterNfts.map((nft) => {
          ...
          {!hasMinimumCakeRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% CCF is required', { num: formatUnits(MINT_COST) })}
            </Text>
          )}
