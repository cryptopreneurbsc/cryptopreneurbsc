...
import { Flex, Skeleton, Text } from '@plaxswap/uikit'
    ...
    <Flex flexDirection="column">
      ...
      <>
        {isLoading || cakeAmount.isNaN() ? (
          ...
          <Balance fontSize="20px" bold unit=" CCF" value={getBalanceNumber(cakeAmount)} decimals={0} />
        )}
        {isLoading || cakeAmount.isNaN() ? (
          <>
          ...
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(cakeAmount.div(parseInt(numberWinners, 10)), 18, 2)} CCF {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
