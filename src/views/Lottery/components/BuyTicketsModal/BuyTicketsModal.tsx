...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      ...
      // If the users' max CCF balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient CCF balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInDexToken.times(new BigNumber(ticketsToBuy))) : '0.00'} CCF`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInDexToken && getFullDisplayBalance(priceTicketInDexToken.times(ticketsToBuy || 0))} CCF
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
