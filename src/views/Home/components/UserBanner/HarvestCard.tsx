...
import { AutoRenewIcon, Button, Card, CardBody, Flex, Skeleton, Text, ArrowForwardIcon } from '@plaxswap/uikit'
  ...
  const harvestAllFarms = useCallback(async () => {
    ...
    for (const farmWithBalance of farmsWithStakedBalance) {
      try {
        ...
        toastSuccess(
          `${t('Harvested')}!`,
          t('Your %symbol% earnings have been sent to your wallet!', { symbol: 'CCF' }),
        )
      } catch (error) {
          ...
          {numTotalToCollect <= 0 ? (
            ...
            >
              <Text color="invertedContrast" bold>
                {pendingTx ? t('Claiming') : t('Claim all')}
              </Text>
            </Button>
          )}
        </Flex>
