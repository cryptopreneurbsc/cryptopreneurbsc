...
const PositionTag: React.FC<PositionTagProps> = ({ betPosition, children, ...props }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'The Locked Price & Closed Price are exactly the same (within 8 decimals), so neither side wins. All funds entered into UP and DOWN positions will go to the weekly CCF burn.',
        )}
      </Text>
    </>,
    { placement: 'right' },
