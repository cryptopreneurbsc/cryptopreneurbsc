...
import { Flex, Box, Text, ExpandableLabel, LinkExternal, Grid, HelpIcon, useTooltip } from '@plaxswap/uikit'
...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of CCF rewards each farm receives, as a proportion of the CCF produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 CCF per block, a 40x farm would receive 40 CCF per block.')}
      </Text>
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            <Text color="textSubtle" small>
              {isFarm ? t('Base APR (CCF yield only)') : t('APR')}
            </Text>
            <Text small textAlign="right">
              {apr.toFixed(2)}%
            </Text>
